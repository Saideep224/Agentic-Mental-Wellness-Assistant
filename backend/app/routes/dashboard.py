...
async def _canonical_profile(db: AsyncSession, user_id: uuid.UUID):
    ...
    if answer_count >= 27 and (not profile or not profile.onboarding_completed or not profile.personality_profile):
...
async def get_dashboard_stats(current_user=Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    ...
    return {"user": {"name": personality.get("name") or current_user.get("name") or "Friend", "email": current_user.get("email")},
            ...
            "emotional_insights": {"current_challenge": personality.get("current_challenge"),
                "support_need": personality.get("primary_support_need"), "emotional_openness": personality.get("emotional_openness"),
                "social_energy": personality.get("social_energy"), "desired_change": personality.get("desired_change"),
                "sleep": emotional.get("sleep"), "life_satisfaction": emotional.get("life_satisfaction")} if completed else {}}
...
async def get_user_profile(current_user=Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    ...
    completed = answer_count >= 27
    if not profile:
        return {"onboarding_completed": completed, "knowing_me_answer_count": answer_count, "personality_profile": {},
                "emotional_baseline": {}, "comfort_preferences": {}, "emotional_style": {}, "stress_triggers": {},
                "preferred_response_style": {}, "emotional_summary": {}}
    return {"onboarding_completed": completed, "knowing_me_answer_count": answer_count,
            "personality_profile": profile.personality_profile or {}, "emotional_baseline": profile.emotional_baseline or {},
            "comfort_preferences": profile.comfort_preferences or {}, "emotional_style": profile.emotional_style or {},
            "stress_triggers": profile.stress_triggers or {}, "preferred_response_style": profile.preferred_response_style or {},
